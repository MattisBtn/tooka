            ...
            <UForm ref="form" :schema="passwordSchema" :state="formData" class="space-y-6" @submit="handleSubmit">
                ...
                <div class="text-center space-y-4">
                    <div class="flex items-center gap-3 justify-center mb-6">
                        <div class="w-12 h-12 bg-amber-500 rounded-xl flex items-center justify-center">
                            <UIcon name="i-lucide-check-square" class="w-6 h-6 text-white" />
                        </div>
                        <div class="text-left">
                    ...
                    <UAlert v-if="hasStoredSession" color="success" variant="soft" icon="i-lucide-clock"
                        ...
                        <template #description>
                            Une session précédente a été trouvée pour cette sélection.
                            Entrez votre code d'accès pour continuer automatiquement.
                        </template>
                    </UAlert>

                    <UAlert v-else color="info" variant="soft" icon="i-lucide-info" title="Sélection protégée">
                        <template #description>
                            Cette sélection est protégée par un code d'accès de 6 caractères.
                            Saisissez le code qui vous a été fourni pour accéder à la sélection d'images.
                        </template>
                    </UAlert>
                </div>
                ...
                <div class="space-y-4">
                    <UButton type="submit" :loading="loading" :disabled="formData.password.length !== 6" color="primary"
                        ...
                        <span v-if="!loading">Accéder à la sélection</span>
...
const hasStoredSession = computed(() => {
    if (!props.selectionId || !import.meta.client) return false;
    try {
        const stored = localStorage.getItem(`selection_auth_${props.selectionId}`);
        return !!stored;
    } catch {
        return false;
    }
