...
const handleStepClick = async (stepNumber: number) => {
    ...
    if (!canViewStep(workflowStep)) {
        const toast = useToast()
        toast.add({
            title: 'Step non accessible',
            description: 'Ce step ne peut pas être consulté car un step plus avancé est déjà configuré.',
    ...
    try {
        await navigateToStep(workflowStep)
