...
export const StatusUtils = {
  ...
  isCompleted(status: Status): boolean {
    return (
      status === PROJECT_STATUS.COMPLETED || status === MODULE_STATUS.COMPLETED
    );
  },
  ...
  isAwaitingClient(status: Status): boolean {
    return (
      status === MODULE_STATUS.AWAITING_CLIENT ||
      status === MODULE_STATUS.PAYMENT_PENDING
    );
  },
  ...
  getNextStatus(
    ...
    action: "submit" | "validate" | "request_revision" | "payment"
  ): ModuleStatus {
    switch (action) {
      case "submit":
        return currentStatus === MODULE_STATUS.DRAFT
          ? MODULE_STATUS.AWAITING_CLIENT
          : currentStatus;
      case "validate":
